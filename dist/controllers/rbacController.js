...
exports.getAccessLogs = exports.checkUserPermission = exports.getUserPermissionsEndpoint = exports.getUserRoles = exports.removeRoleFromUser = exports.assignRoleToUser = exports.getRolePermissions = exports.assignPermissionsToRole = exports.getPermissionsByModule = exports.getPermissions = exports.updateRole = exports.createRole = exports.getRoleById = exports.getRoles = void 0;
...
const getRoles = async (req, res) => {
    try {
        const result = await database_1.default.query(`SELECT id, name, code, description, level, is_active
       FROM roles
...
const createRole = async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO roles (name, code, description, level)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [name, code, description, level]);
...
const updateRole = async (req, res) => {
    try {
        ...
        const checkResult = await database_1.default.query('SELECT is_system_role FROM roles WHERE id = $1', [id]);
        if (checkResult.rows[0]?.is_system_role) {
            return res.status(403).json({ message: 'Impossible de modifier un rôle système' });
        }
        const result = await database_1.default.query(`UPDATE roles
       SET name = $1, description = $2, level = $3, is_active = $4
       WHERE id = $5
       RETURNING *`, [name, description, level, is_active, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: 'Rôle non trouvé' });
        }
        res.json(result.rows[0]);
...
const getPermissionsByModule = async (req, res) => {
    try {
        const result = await database_1.default.query(`SELECT module, json_agg(json_build_object(
        'id', id,
        'code', code,
        'name', name,
        'action', action,
...
const assignPermissionsToRole = async (req, res) => {
    ...
    try {
        ...
        if (permissionIds && permissionIds.length > 0) {
            const values = permissionIds.map((permId, index) => `($1, $${index + 2}, $${permissionIds.length + 2})`).join(',');
            await client.query(`INSERT INTO role_permissions (role_id, permission_id, granted_by)
         VALUES ${values}`, [roleId, ...permissionIds, userId]);
        }
...
const getRolePermissions = async (req, res) => {
    try {
        const { roleId } = req.params;
        const result = await database_1.default.query(`SELECT p.*
       FROM role_permissions rp
       JOIN permissions p ON rp.permission_id = p.id
       WHERE rp.role_id = $1
...
const assignRoleToUser = async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO user_roles (user_id, role_id, assigned_by, expires_at)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id, role_id) 
       DO UPDATE SET is_active = true, expires_at = $4
       RETURNING *`, [userId, roleId, assignedBy, expiresAt || null]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) {
...
const removeRoleFromUser = async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`DELETE FROM user_roles
...
const getUserRoles = async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`SELECT r.*, ur.assigned_at, ur.expires_at, ur.is_active
       FROM user_roles ur
       JOIN roles r ON ur.role_id = r.id
       WHERE ur.user_id = $1
       ORDER BY r.level`, [userId]);
        res.json(result.rows);
    }
    catch (error) {
...
const getUserPermissionsEndpoint = async (req, res) => {
    try {
        ...
        const permissions = await (0, rbac_1.getUserPermissions)(userId);
...
exports.getUserPermissionsEndpoint = getUserPermissionsEndpoint;
...
const checkUserPermission = async (req, res) => {
    try {
      ...
      ) as has_permission`, [userId, permission]);
        res.json({
            permission,
