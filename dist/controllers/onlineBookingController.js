...
exports.convertToInternalBooking = exports.getOnlineBookingStats = exports.getAllOnlineBookings = exports.getFAQs = exports.getPublicReviews = exports.cancelOnlineBooking = exports.getBookingByReference = exports.createOnlineBooking = exports.validatePromoCode = exports.getBookingSettings = exports.getAvailableRooms = void 0;
...
const getAvailableRooms = async (req, res) => {
    try {
        ...
        const roomTypes = await database_1.default.query(`SELECT DISTINCT 
        type,
        price,
       ...
       ORDER BY price`);
        ...
        const availability = await Promise.all(roomTypes.rows.map(async (room) => {
            const availResult = await database_1.default.query('SELECT get_room_availability($1, $2, $3) as available', [room.type, check_in, check_out]);
            return {
                type: room.type,
...
const getBookingSettings = async (req, res) => {
    try {
        const result = await database_1.default.query('SELECT * FROM online_booking_settings LIMIT 1');
...
exports.getBookingSettings = getBookingSettings;
// Validate promo code
const validatePromoCode = async (req, res) => {
    try {
        const { code, room_type, check_in, check_out, total_amount } = req.body;
        const result = await database_1.default.query(`SELECT * FROM promo_codes 
       WHERE code = $1 
       AND is_active = true
       AND valid_from <= CURRENT_DATE
       AND valid_until >= CURRENT_DATE
       AND (max_uses IS NULL OR current_uses < max_uses)`, [code.toUpperCase()]);
        if (result.rows.length === 0) {
...
const createOnlineBooking = async (req, res) => {
    ...
    try {
        const { guest_first_name, guest_last_name, guest_email, guest_phone, guest_country, guest_address, guest_city, guest_postal_code, room_type, check_in_date, check_out_date, number_of_guests, special_requests, arrival_time, promo_code } = req.body;
        ...
        // Get room price
        const roomResult = await client.query('SELECT price FROM rooms WHERE type = $1 LIMIT 1', [room_type]);
        ...
        let discount = 0;
        if (promo_code) {
            const promoResult = await client.query(`SELECT * FROM promo_codes 
         WHERE code = $1 AND is_active = true
         AND valid_from <= CURRENT_DATE AND valid_until >= CURRENT_DATE`, [promo_code.toUpperCase()]);
        ...
        const settingsResult = await client.query('SELECT deposit_percentage, require_deposit FROM online_booking_settings LIMIT 1');
        ...
        // Create online booking
        const bookingResult = await client.query(`INSERT INTO online_bookings (
...
exports.createOnlineBooking = createOnlineBooking;
// Get booking by reference (for customer to check status)
const getBookingByReference = async (req, res) => {
    try {
        const { reference } = req.params;
        const { email } = req.query;
        const result = await database_1.default.query('SELECT * FROM online_bookings WHERE booking_reference = $1 AND guest_email = $2', [reference, email]);
        if (result.rows.length === 0) {
...
const cancelOnlineBooking = async (req, res) => {
    try {
        const { reference } = req.params;
        const { email, reason } = req.body;
        const result = await database_1.default.query(`UPDATE online_bookings 
       ...
       RETURNING *`, [reason, reference, email]);
        if (result.rows.length === 0) {
...
exports.cancelOnlineBooking = cancelOnlineBooking;
// Get public reviews
const getPublicReviews = async (req, res) => {
    try {
        const { limit = 10 } = req.query;
        const result = await database_1.default.query(`SELECT guest_name, rating, comment, room_type, stay_date, response, responded_at
       FROM public_reviews
...
exports.getAllOnlineBookings = getAllOnlineBookings;
// Get online booking statistics (admin)
const getOnlineBookingStats = async (req, res) => {
    try {
        const result = await database_1.default.query('SELECT * FROM v_online_booking_stats');
        res.json(result.rows[0]);
    }
    catch (error) {
...
exports.getOnlineBookingStats = getOnlineBookingStats;
// Convert online booking to internal booking (admin)
const convertToInternalBooking = async (req, res) => {
    const client = await database_1.default.connect();
    try {
        const { id } = req.params;
        ...
        await client.query('BEGIN');
        // Get online booking
        const onlineBooking = await client.query('SELECT * FROM online_bookings WHERE id = $1', [id]);
        ...
        // Create or get guest
        let guestResult = await client.query('SELECT id FROM guests WHERE email = $1 OR phone = $2', [ob.guest_email, ob.guest_phone]);
        let guestId;
        if (guestResult.rows.length === 0) {
            // Create new guest
            guestResult = await client.query(`INSERT INTO guests (first_name, last_name, email, phone, address, city, country)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING id`, [ob.guest_first_name, ob.guest_last_name, ob.guest_email, ob.guest_phone,
        ...
        const roomResult = await client.query(`SELECT id FROM rooms 
       WHERE type = $1 
       ...
       LIMIT 1`, [ob.room_type, ob.check_in_date, ob.check_out_date]);
        if (roomResult.rows.length === 0) {
            await client.query('ROLLBACK');
      ...
      RETURNING *`, [guestId, roomId, ob.check_in_date, ob.check_out_date, ob.number_of_guests,
            'confirmed', ob.total_amount, ob.special_requests, userId]);
        // Update online booking
        await client.query(`UPDATE online_bookings 
       SET status = 'converted',
           converted_to_booking_id = $1,
           converted_at = CURRENT_TIMESTAMP,
           converted_by = $2
       WHERE id = $3`, [bookingResult.rows[0].id, userId, id]);
        await client.query('COMMIT');
        res.json({
...
exports.convertToInternalBooking = convertToInternalBooking;
