...
router.put('/settings', async (req, res) => {
    try {
        const userId = req.user.id;
        const { hotel_name, hotel_address, hotel_city, hotel_phone, hotel_email, time_zone, email_notifications, booking_alerts, payment_notifications, theme, language, signature, } = req.body;
        console.log('💾 Saving settings for user:', userId);
        console.log('Signature length:', signature ? signature.length : 0);
        // Check if settings exist
        const existing = await database_1.default.query('SELECT id FROM user_settings WHERE user_id = $1', [userId]);
        if (existing.rows.length === 0) {
            // Insert new settings
            const result = await database_1.default.query(`INSERT INTO user_settings 
         ...
         RETURNING *`, [userId, hotel_name, hotel_address, hotel_city, hotel_phone, hotel_email,
                time_zone, email_notifications, booking_alerts, payment_notifications, theme, language, signature || '']);
            console.log('✅ Settings created');
            return res.json(result.rows[0]);
        }
        // Update existing settings
        const result = await database_1.default.query(`UPDATE user_settings 
       ...
       WHERE user_id = $13
       RETURNING *`, [hotel_name, hotel_address, hotel_city, hotel_phone, hotel_email,
...
router.put('/profile-picture', async (req, res) => {
    try {
        ...
        await database_1.default.query('UPDATE users SET profile_picture = $1, updated_at = NOW() WHERE id = $2', [profilePicture, userId]);
